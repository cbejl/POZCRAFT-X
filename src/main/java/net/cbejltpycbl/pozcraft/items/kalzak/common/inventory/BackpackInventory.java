...
public class BackpackInventory implements Inventory {
    ...
    public void write() {
        if (this.container != null && !this.container.isEmpty()) {
            this.container.getOrCreateNbt().put("kalzak", Inventories.writeNbt(new NbtCompound(), this.list, true));
        }
    }

    public void read() {
        if (this.container != null && !this.container.isEmpty()) {
            Inventories.readNbt(this.container.getOrCreateNbt().getCompound("kalzak"), this.list);
        }
    }
}
